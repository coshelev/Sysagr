...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Об.ЗаполниТЧ_АнкетыРекурсивно();
	Об.ЗаполниТЧ_ВыбираемыеОтветы();
	ЗначениеВРеквизитФормы(Об, "Объект");

	//<отображение телефона внешнего абонента и канала звонка>
	// телефон внешнего абонента
	РегСсылка = Конвертация.РегионПолучитьПоНомеруТелефона(ЭтаФорма.Параметры.Телефон);
	ЭтаФорма.Элементы.Дисплей01.Заголовок = ЭтаФорма.Параметры.Телефон;
	Если (ЗначениеЗаполнено(РегСсылка)) Тогда
		Темп = СокрЛП(ЭтаФорма.Параметры.Телефон) + " - " + СокрЛП(РегСсылка);
		Элементы.Дисплей01.Заголовок = СокрЛП(Темп);
	КонецЕсли;

	//отображение канала звонка
	ТелСсылка = Конвертация.ТелВнешнийПолучитьСсылку(ЭтаФорма.Параметры.КаналИнициатор);
	КодГорода = "";
	...
	//<09.04.2018 11-41,+> 
	//отображение сайта-отправителя автозвонка
	//-------------------------------------------------------------
	Если ЗначениеЗаполнено(ЭтаФорма.Параметры.СайтОтправительАвтозвонка) Тогда
		Элементы.Дисплей03.Заголовок = СокрЛП(ЭтаФорма.Параметры.СайтОтправительАвтозвонка);		
	КонецЕсли;
	//</09.04.2018 11-41> 
	...
	//<09.04.2018 11-41,+> 
	//отображение сайта-отправителя автозвонка (исходящий звонок от оператора КЦ)
	//----------------------------------------------------------------------------
	Если ЗначениеЗаполнено(ЭтаФорма.Параметры.СайтОтправительАвтозвонка) Тогда
		...
		Если ЗначениеЗаполнено (ПервыйВопрос) Тогда
			...
			//Запрос.УстановитьПараметр("Ответ", "%Сайты%");
			//
			...
			//Если ЗначениеЗаполнено(ПервыйВопрос) Тогда
				...
				Запрос.УстановитьПараметр("ПервыйВопрос", 		ПервыйВопрос);
				Запрос.УстановитьПараметр("Ответ", ЭтаФорма.Параметры.СайтОтправительАвтозвонка);
	...
	Иначе //Входящий звонок
		...
		//Для входящего определеи регион звонящего абонента
		//---------------------------------------------------
		Если ЗначениеЗаполнено (ПервыйВопрос) И ЗначениеЗаполнено(ЭтаФорма.Параметры.Телефон) Тогда
			РегСсылка = Конвертация.РегионПолучитьПоНомеруТелефона(ЭтаФорма.Параметры.Телефон);
...
Процедура Предприятие_ТелефоныВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если (Поле.Имя = "Предприятие_ТелефоныAtxfer") Тогда
		Результат = КонтактЦентр._КомандаAtxfer(ЭтаФорма.Параметры.КаналСистемный, ТелНомер);
	ИначеЕсли (Поле.Имя = "Предприятие_ТелефоныRedirect") Тогда
		Результат = КонтактЦентр.Команда_BlindTransfer(ЭтаФорма.Параметры.КаналСистемный, ТелНомер);
	КонецЕсли;
...
Процедура АнкетаТекущийВопросВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗначенияОтвета = Тип("СправочникСсылка.ТелВнутренние") 
		...
		Результат = КонтактЦентр._КомандаAtxfer(ЭтаФорма.Параметры.КаналСистемный, СледюущееДействиеКод);
	...
	ИначеЕсли ТипЗначенияОтвета = Тип("СправочникСсылка.ТелОчереди")
		...
		Результат = КонтактЦентр._КомандаBlindTransfer(ЭтаФорма.Параметры.КаналСистемный, СледюущееДействиеКод);
		ТекстСообщения = ?(Результат,"Перевод выполнен","Ошибка перевода звонка");
		ПоказатьПредупреждение(,ТекстСообщения,3);
		КонтактЦентрСервер.ПереводЗвонка(ТекущийПользователь, СледующееДействие);
		ЭтаФорма.Закрыть();
	ИначеЕсли ТипЗначенияОтвета = Тип("СправочникСсылка.ВиртуальныеОчереди") Тогда
		...
		Результат = КонтактЦентр._КомандаBlindTransfer(ЭтаФорма.Параметры.КаналСистемный, Агент.Телефон);
		ТекстСообщения = ?(Результат,"Перевод выполнен","Ошибка перевода звонка");
		ПоказатьПредупреждение(,ТекстСообщения,3);
