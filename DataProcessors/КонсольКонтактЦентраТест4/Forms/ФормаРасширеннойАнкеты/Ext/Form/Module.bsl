...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено (Параметры.КаналИнициатор) И ТипЗнч(Параметры.КаналИнициатор) = Тип("СправочникСсылка.ТелВнешние") Тогда
		...
		Запрос = Новый Запрос();
		Запрос.Текст = 
		"ВЫБРАТЬ Ссылка, СледующийВопросИлиТелНомер
	...
	Если ЗначениеЗаполнено(Параметры.СайтИнтернетЗаявки) Тогда
		...
		Запрос = Новый Запрос();
		Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1 СледующийВопросИлиТелНомер
		...
		Если ЗначениеЗаполнено (ПервыйВопрос) Тогда
				...
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ ПЕРВЫЕ 1 СледующийВопросИлиТелНомер
				|ИЗ			Справочник.Анкеты.ВариантыОтветов КАК АнкетыВариантыОтветов
				|ГДЕ
				|	НЕ Ссылка.ЭтоГруппа
				|	И Ответ = &Ответ
				|	И Ссылка В (ВЫБРАТЬ ВТ01.ВопросВОтвете ИЗ ВТ01)";
				...
				Если Не РезультатЗапроса.Пустой() Тогда
					...
					Доступно = СтрНайти(ПервыйВопрос.Наименование, "Регион абонента")> 0
							И ЗначениеЗаполнено(Параметры.Регион)
					...
					Если Доступно Тогда
						...
						Запрос = Новый Запрос();
						Запрос.Текст = 
						"ВЫБРАТЬ ПЕРВЫЕ 1 СледующийВопросИлиТелНомер КАК СледующийВопросИлиТелНомер
						|ИЗ Справочник.Анкеты.ВариантыОтветов КАК АнкетыВариантыОтветов
						|ГДЕ
						|	НЕ Ссылка.ЭтоГруппа
						|	И Ответ ПОДОБНО &Ответ";           
...
Процедура Назад(Команда)
	...
	Объект.АнкетаТекущийВопрос.Очистить();
	Отб = Новый Структура();
	Отб.Вставить("Вопрос", ПервыйВопросДляВозвратаДомой);
	Найденные = Объект.АнкетаОтветы.НайтиСтроки(Отб);
	Для Каждого Стр Из Найденные Цикл
		Новая = Объект.АнкетаТекущийВопрос.Добавить();
		ЗаполнитьЗначенияСвойств(Новая, Стр);
