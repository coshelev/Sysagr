...
Процедура ПриСозданииНаСервере(Отказ,СтандартнаяОбработка)
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Т.Ссылка КАК Ссылка,
	|	Т.Ссылка.Очередь КАК Очередь,
	|	Т.Ссылка.Очередь.Сервер.ИсточникДанных КАК Сервер,
	|	Т.Ссылка.ИспользуетсяКаскадОчередей КАК ИспользуетсяКаскадОчередей
	|ИЗ
	|	Справочник.КонтактЦентры.Пользователи КАК Т
	|ГДЕ
	|	Т.Пользователь = &Пользователь
	|
	|УПОРЯДОЧИТЬ ПО
	|	Т.Ссылка.Наименование,
	|	Т.Ссылка.Очередь.Наименование";
