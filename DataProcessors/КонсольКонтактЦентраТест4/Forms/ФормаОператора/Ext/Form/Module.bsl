...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Об.Заполни_КэшСайтыТребующиеАвтоответаАнкетыНаВопросРегионАбонента();
...
Процедура _ВнешнееСобытие(Источник,Событие,Данные)
	...
	Доступно = 		ТипЗнч(Событие) = Тип("Строка")
				И ВРег(СокрЛП(Событие) = "DISCONNECT");
	Если Доступно Тогда
		ВнешнееСобытиеHangup();
		Возврат;
	КонецЕсли;	
...
Процедура ПереключитьЗвонокПоРасширеннойАнкете(Команда)
	...
	Доступно = ЗначениеЗаполнено(Парам.СайтИнтернетЗаявки) И СтрНайти(ЭтаФорма.Объект.КэшСайтыТребующиеАвтоответаАнкетыНаВопросРегионАбонента, Парам.СайтИнтернетЗаявки)>0;
	...
	Парам.Вставить("ИнтернетЗаявкаСвойства", ИнтернетЗаявкаСвойства);
...
//-------------------------------------------------------------------------------------------------
	...
	ИнтернетЗаявкаСвойства				= ТелефонИСигнатураОткрытойЗадачи.Свойства;
	ДисплейИмяАвтозвонка 				= "";
	ДисплейМайлАвтозвонка 				= "";
...
Функция ТелефонИСигнатураИнтернетЗаявки(КонсольТелефон) 	
	//Возвращает структуру в которой поле "Телефон" содержит телефон абонента, поле "Сигнатура" содержит сигнатуру интернет-заявки, поле "Сайт" содержит сайт
...
Процедура ЗадачиСписокЗвонкиПриАктивизацииСтроки(Элемент)
	...
	Окончание = ТекущаяДата();
	Начало = ДобавитьМесяц(Окончание, -2);
	СформируйИсториюОбщения(СокрЛП(ТелНомер), Начало, Окончание, Истина, ЭтаФорма.КлючСкрытогоОбсужденияСистемыВзаимодействия, xmlСтрока(ЭтаФорма.УникальныйИдентификатор));
