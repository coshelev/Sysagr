...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Загрузи историю общения из родительской формы
	//---------------------------------------------
	Если Параметры.ИсторияОбщения <> "" Тогда
		ТЗ = ПолучитьИзВременногоХранилища(Параметры.ИсторияОбщения);
	...
	Если ЗначениеЗаполнено(Параметры.СайтИнтернетЗаявки) Тогда	
		Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 Т.Сигнатура ИЗ РегистрСведений.ЗадачиИнтернетЗаявки ГДЕ Телефон = &Телефон И ДатаЗакрытия = ДАТАВРЕМЯ(1, 1, 1) И Заблокирована = ИСТИНА");
