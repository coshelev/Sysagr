...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)	
	МоиНастройки_ТолькоЗвонкиПоЗадачеВИсторииОбщения 	= Истина;
	Элементы.МоиНастройки_ТолькоЗвонкиПоЗадачеВИсторииОбщения.Видимость = Ложь;
	МоиНастройки_ИспользоватьАвтоподбор 				= Истина;	
	
	Элементы.КорректировкаСферы.Видимость 				= РольДоступна("Администратор")	Или РольДоступна("КонтактЦентр_Администратор");
...
Процедура ИнформацияПредыдущейЗадачи(ИмяАктивногоДинамическогоСписка, Телефон, Автомобиль);
	...
	Запрос = Новый Запрос("ВЫБРАТЬ Т.ДатаЗакрытия КАК ДатаЗакрытия, Т.ПричинаЗакрытия КАК ПричинаЗакрытия ИЗ РегистрСведений.ЗадачиЗвонокЛояльности КАК Т ГДЕ Т.СигнатураПродажи = &СигнатураПродажи");
...
Процедура ЗадачиСписокЛояльностьРемонтаПриАктивизацииСтроки(Элемент)
	...
	//---------------------------------------------------------
	СформируйПодобныеЗадачи(ТекДанные.Автомобиль, ТекДанные.СигнатураПродажи, ТекДанные.МашинозаездСсылка);
	...
	//-------------------------------------------------------------------------
	ИнформацияПредыдущейЗадачи(ИмяЭлемента, ТекДанные.Телефон, ТекДанные.Автомобиль);	
КонецПроцедуры
...
Функция СформируйПодобныеЗадачи(Автомобиль, СигнатураМастерПродажи, МашинозаездСсылка = "");
	...
	|	И ЗадачиЗвонокЛояльности.СигнатураПродажи <> &СигнатураМастерПродажи
	|	И ЕСТЬNULL(Продажи.Объект, """") = &Автомобиль
	|	И Продажи.ДопДата <> ДАТАВРЕМЯ(1, 1, 1)";
...
Процедура ПоказатьАнкетуПредыдущейЗадачиНаСервере()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ Т.Сигнатура КАК Сигнатура ИЗ РегистрСведений.ЗадачиЛояльностиПакеты КАК Т ГДЕ Т.Продажа = &Продажа");
...
&НаКлиенте
Процедура КорректировкаСферы(Команда)
