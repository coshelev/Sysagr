...
Функция СтартуйБП(СигнатураПродажи, Дата, ДатаСтарта, ТочкаПродажи, Телефон, ПодразделениеИсполнитель, ПодразделениеКонтролер = Неопределено, ДопРеквизитАдресации = "")
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ОбратнаяСвязьЛояльности.Ссылка КАК Ссылка
	|ИЗ
	|	БизнесПроцесс.ОбратнаяСвязьЛояльностиПродаж КАК ОбратнаяСвязьЛояльности
	|ГДЕ
	|	ОбратнаяСвязьЛояльности.Стартован = ИСТИНА
	|	И ОбратнаяСвязьЛояльности.Завершен = ЛОЖЬ
	|	И ОбратнаяСвязьЛояльности.СигнатураПродажи = &СигнатураПродажи";
	...
	//-------------------------------------------------------------
	БП 					= БизнесПроцессы.ОбратнаяСвязьЛояльностиПродаж.СоздатьБизнесПроцесс();
	БП.Дата 			= Дата;
	БП.ДатаСтарта		= Дата;
