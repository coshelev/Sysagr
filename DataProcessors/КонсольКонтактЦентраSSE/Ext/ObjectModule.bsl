...
Процедура Заполни_КэшСайтыТребующиеАвтоответаАнкетыНаВопросРегионАбонента() Экспорт
	//
	//Об =  Обработки.ТаблицаXML_СайтыТребующиеАвтоответаАнкетыНаВопросРегионАбонента.Создать();
	//Об.ЗначениеИзмерения	= "СайтыТребующиеАвтоответаАнкетыНаВопросРегионАбонента"; 
	//Об.УстановитьНастройки();  
	//
	////Возвращает массив имен сайтов
	////----------------------------
	//Рез = Об.Десериализовать(); 
	//
	//Стр="";
	//Для Каждого i Из Рез Цикл
	//	Стр = Стр+";"+i.Сайт;
	//КонецЦикла; 
	
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ Наименование ИЗ Справочник.Сайты ГДЕ ПриЗвонкеТребуетсяАвтоответНаВопросРегионАбонента = Истина");
	РезультатЗапроса = Запрос.Выполнить(); 
	Если РезультатЗапроса.Пустой() Тогда
		Возврат;
	КонецЕсли;
	ЭтотОбъект.КэшСайтыТребующиеАвтоответаАнкетыНаВопросРегионАбонента = СтрСоединить(РезультатЗапроса.Выгрузить().ВыгрузитьКолонку(0), ";");
КонецПроцедуры
