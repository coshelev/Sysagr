...
Процедура ЗаполниТаблицуАнкетуОпроса(СигнатураПродажи)
	...
	|ИЗ
	|	РегистрСведений.АнкетыРасширенные КАК Т
	|ГДЕ
	|	Т.СигнатураЗадачQ = &СигнатураЗадачи
	|
	|УПОРЯДОЧИТЬ ПО
	|	Т.ПорядковыйНомерВопроса";
...
&НаСервере
Процедура ЗаполниТаблицуВопросовТребующихВводОтветов(СигнатураПродажи)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ШаблонАнкетыОбрСвязи.Ссылка, ЗНАЧЕНИЕ(Справочник.Анкеты.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.Анкеты.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ВопросСвязанСОтветомАнкетыОбратнойСвязи
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Анкеты.ВариантыОтветов КАК ШаблонАнкетыОбрСвязи
	|		ПО Анкеты.Ссылка = ШаблонАнкетыОбрСвязи.СледующийВопросИлиТелНомер
	|ГДЕ
	...
	|ИЗ
	|	РегистрСведений.АнкетыОбратнойСвязи.СрезПоследних(, СигнтраЗадачи = &СигнатураПродажи) КАК РегСрез
	|
	|СГРУППИРОВАТЬ ПО
	|	РегСрез.Вопрос
	|;
	...
	|	ВТ00.ВопросСвязанСОтветомАнкетыОбратнойСвязи КАК ВопросСвязанСОтветомАнкетыОбратнойСвязи,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АнкетыОбратнойСвязи.СрезПоследних(, СигнтраЗадачи = &СигнатураПродажи) КАК РегСрез
	...
	Запрос.УстановитьПараметр("НаименованиеГруппы", "ОБРАТНАЯ_СВЯЗЬ_РЕМОНТ");
	...
	Пока Выборка.Следующий() Цикл
		...
		//Ищем связку этого вопроса с ответом в анкете обратной связи
		//-----------------------------------------------------------
		Если Выборка.ВопросСвязанСОтветомАнкетыОбратнойСвязи Тогда
			
			//Есть в анкете обр.связи ответ с этой сферой негатива?
			//-----------------------------------------------------
			З = Новый Запрос();
			З.Текст = 
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	АнкетыОбрСвязи.СигнатураЗадачQ КАК СигнатураЗадачQ
			|ИЗ
			|	РегистрСведений.АнкетыРасширенные КАК АнкетыОбрСвязи
			|ГДЕ
			|	АнкетыОбрСвязи.СигнатураЗадачQ = &СигнатураЗадачи
			|	И АнкетыОбрСвязи.Ответ = &Ответ";
			З.УстановитьПараметр("СигнатураЗадачи", СигнатураПродажи);
			З.УстановитьПараметр("Ответ",			Выборка.ИдВопроса);
			Если З.Выполнить().Пустой() Тогда
				//Не этой сферы негатива в анкете обр.связи, значит не отображаем вопрос по ней в анкете обработки негатива
				//-----------------------------------------------------------------------------------------------------------
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Процедура ТаблицаВведенныхОтветовОтветКомментарийСтрокойОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	ТекДанные = Элементы.ТаблицаВведенныхОтветов.ТекущиеДанные;	
	ТекДанные.ФорматТекстаОтвета = 2;
	ДобавитьЗаписьВТаблицуВопросОтвет(ТекДанные.ВопросАнкеты, ТекДанные.ОтветСтрокой, Текст);
КонецПроцедуры
...
//<Запись анкеты>
&НаКлиенте
Процедура ЗаписатьАнкету(Команда = Неопределено)
	...
	Доступно = Истина;
	Доступно = Доступно И ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ОбработатьНегативнуюАнкету_1");
	...
	Если ИмяАктивногоДинамичСписка = "ЗадачиСписокЛояльностьРемонта" Тогда
		...
		Если Не Выполнена Тогда
			ПоказатьПредупреждение(,"Ошибка закрытия задачи "+ЗадачаСсылка);
		КонецЕсли;
		
		Если Выполнена Тогда
			ЗакройОткрытыеОбращения(ЗадачаСсылка);
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЗакройОткрытыеОбращения(ЗадачаОснование)
	//Помечает на удаление процессы типа обращения	
	Возврат "ок"
КонецФункции
...
Функция ЗаписатьВыбранныйОтвет_НаСервере(ДатаАнкеты, ТелАбонента, СигнатураЗадачи)
	...
	НЗ.Отбор.СигнтраЗадачи.Установить(СигнатураЗадачи);
	НЗ.Прочитать();
...
Процедура ЗадачиСписокЛояльностьРемонтаПриАктивизацииСтроки(Элемент=Неопределено)
	...
	//--------------------------------
	ТаблицаВведенныхОтветов.Очистить();
	ЗаполниТаблицуВопросовТребующихВводОтветов(ТекДанные.СигнатураПродажи);	
	...
	Если  Доступно Тогда
		...
		Если 	ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ОбработатьНегативнуюАнкету_1")
			Или ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ОбработатьНегативнуюАнкету_2")
			Или ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ОбработатьНегативнуюАнкету_3")
			Или ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ОбработатьНегативнуюАнкету_4")
			Или ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ОбработатьНегативнуюАнкету_5")
		Тогда
			Элементы.КнЗаписатьАнкету.Доступность = Истина;	
		КонецЕсли;
			
		Если 	ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ПроверитьОбработаннуюНегативнуюАнкету_1")
			Или ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ПроверитьОбработаннуюНегативнуюАнкету_2")
			Или ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ПроверитьОбработаннуюНегативнуюАнкету_3")
			Или ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ПроверитьОбработаннуюНегативнуюАнкету_4")
			Или ТочкаМаршрута = ПредопределенноеЗначение("БизнесПроцесс.ОбратнаяСвязьЛояльности.ТочкаМаршрута.ПроверитьОбработаннуюНегативнуюАнкету_5")
		Тогда
...
Процедура БизнесПроцессИсторияНаСервере(БизнесПроцессСсылка)
	...
	Запрос = Новый Запрос();
	Запрос.Текст = 
	"ВЫБРАТЬ Ссылка КАК Задача, Выполнена, ИсполнительФакт КАК Исполнитель, Комментарий, Дата КАК ДатаПостановки, ДатаВыполнения
	|ИЗ 	Задача.ЗадачаОбратнаяСвязь
...
Процедура Печать_НаСервере(СигнатураПродажи, ТаблДок)
	Отч = Отчеты.СТО_ПолныйРеестрНегативныхОтзывовРекомендацийПоДатеЗакрытияЗадачи_v122020.Создать();
	Отч.УстановитьНастройки(Ложь);
