...
Процедура ОтправитьПисьмо()
	...
	// 1. Отправь почтовое сообщение
	// 2. Зарегистрируй факт отправки в Агр
	// 3. Закрой задачу
...
Функция ОтправитьПочтовоеСообщение()
	...
	ЗакройЗадачу(Параметры.ВходящееПисьмоОснование);
...
&НаСервере
Функция ЗакройЗадачу(ВходящееПисьмоОснование)
	...
	|ГДЕ
	|	Т.ПисьмоИнициатор = &ПисьмоИнициатор
	...
	РегистрыСведений.ЗадачиОбратныйЗвонок.УстановиЗначениеРеквизитаИЗакройЗадачу(Телефон , ТекущаяДата(), "ПричинаЗакрытия", "Пересылка из Агр", ПараметрыСеанса.ТекущийПользователь);
