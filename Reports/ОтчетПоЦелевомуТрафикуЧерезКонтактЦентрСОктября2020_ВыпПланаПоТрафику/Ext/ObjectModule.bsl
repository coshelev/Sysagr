Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ИначеЕсли ЭтотОбъект.ВыбранныйВариантОтчета = "СводныйКаналыСайты" Тогда
...
Процедура ПриКомпоновкеРезультатаСводный(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	|	Т.Направление КАК НазваниеНаправления,
	...
	|	ВТ018.НазваниеНаправления КАК НазваниеНаправления,
	...
	|			И ВТ020.НазваниеНаправления = ВТ018.НазваниеНаправления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТ025.Направление КАК НазваниеНаправления,
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""ГАЗ""
	|			ТОГДА 10
	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""УАЗ""
	|			ТОГДА 20
	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""ВАЗ""
	|			ТОГДА 30
	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""Ford""
	|			ТОГДА 40
	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""Chery""
	|			ТОГДА 41
	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""КАМАЗ""
	|			ТОГДА 42
	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""СПЕЦТЕХНИКА""
	|			ТОГДА 43 
	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""Kangaroo Electro""
	|			ТОГДА 44 
    |		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""МАЗ""
	|			ТОГДА 45
    |		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""МТЗ""
	|			ТОГДА 46   
  	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""С пробегом""
	|			ТОГДА 47
  	|		КОГДА ЕСТЬNULL(ВТ021.НазваниеНаправления, ВТ030.НазваниеНаправления) = ""<Нет направления>"" 	
	|			ТОГДА 50
	|	КОНЕЦ КАК ПорядокНаправлений
	|ИЗ
	|	ВТ021 КАК ВТ021
...
Функция СделкиВстречиВыставкиОУ(Начало, Окончание)
	...
	|		КОГДА СделкаРеализации.ВидДеятельности = ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.НовыеСПЕЦ) ТОГДА ""СПЕЦТЕХНИКА""
	|		КОГДА СделкаРеализации.ВидДеятельности = ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.НовыеКАМАЗ) ТОГДА ""КАМАЗ""  
	|		КОГДА СделкаРеализации.ВидДеятельности = ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.НовыеМТЗ) ТОГДА ""МТЗ"" 
	|		КОГДА СделкаРеализации.ВидДеятельности = ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.НовыеМАЗ) ТОГДА ""МАЗ""  
    |		КОГДА СделкаРеализации.ВидДеятельности = ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.НовыеKangarooElectro) ТОГДА ""Kangaroo Electro""  
