						...
						Пока ВыбТелефон.Следующий() Цикл
							...
							Пока ВыбКонтакт.Следующий() Цикл
								...
								Пока ВыбСигнатура.Следующий() Цикл
									...
									|ГДЕ
									|	ЗадачаОбратнаяСвязь.БизнесПроцесс.СигнатураПродажи = &СигнатураПродажи
									|	И НЕ ЗадачаОбратнаяСвязь.Комментарий ПОДОБНО &Комментарий";
									
									Запр.УстановитьПараметр("СигнатураПродажи",	ВыбСигнатура.СигнатураЗадачи);
									Запр.УстановитьПараметр("Комментарий", "");	
									тз 			= Запр.Выполнить().Выгрузить();
									Если тз.Количество()>0 Тогда
