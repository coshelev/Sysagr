...
Функция Сводный()
	...
	|				И Вопрос В (&_1йБазовыйВопросЛояльности, &_2йБазовыйВопросЛояльности, &_3йБазовыйВопросЛояльности, &_4йБазовыйВопросЛояльности)) КАК ВТ01_АнкетыРасш
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СигнатураЗадачи,
	|	Вопрос,
	...
	|	ВЫБОР
	|		КОГДА ЗадачаЛояльности.ПричинаЗакрытия = """"
	|				И НЕ _1йБазовыйВопросЛояльности.Ответ В (&_1, &_2, &_3)
	|				И НЕ _2йБазовыйВопросЛояльности.Ответ В (&_1, &_2, &_3)
	|				И НЕ _3йБазовыйВопросЛояльности.Ответ В (&_1, &_2, &_3)
	|				И НЕ _4йБазовыйВопросЛояльности.Ответ В (&_1, &_2, &_3)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ01_АнкетыРасш КАК _3йБазовыйВопросЛояльности
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ01_АнкетыРасш КАК _4йБазовыйВопросЛояльности
	...
	|ИЗ
	|	ВТ00_СигнатураЗамененаНаМастер КАК ВТ00
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АнкетыРасширенные.СрезПоследних(
	|				,
	|				СигнатураЗадачQ В
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|						ВТ00.СигнатураПродажи
	...
	|							ПО
	|								ВТ00.СигнатураПродажи = Задачи.СигнатураПродажи
	|									И Задачи.ПричинаЗакрытия = """")) КАК ПМЗ
	|		ПО ВТ00.СигнатураПродажи = ПМЗ.СигнатураЗадачQ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ИдентификаторыСтрок КАК ИдС
	|		ПО (ПМЗ.Ответ = ИдС.Ссылка)
	|			И (ПОДСТРОКА(ИдС.ПолнаяСтрока, 1, 3) = ""ПМЗ"")
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ00.Точка,
	|	ВТ00.СигнатураПродажи
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ031.Точка КАК Точка,
	|	ВТ031.СигнатураПродажи КАК СигнатураПродажи,
	|	ВЫБОР
	|		КОГДА ВТ031.Количество > 0
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Количество
	|ПОМЕСТИТЬ ВТ032
