...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	|	ВТ05.Принят КАК Принят,
	|	ВТ05.Направление КАК Направление,    
	|	Звонки.Инициатор.Код + "" "" + Звонки.Инициатор.Назначение КАК КаналНомерНазначение,
	|	Стат.Основание КАК Основание,
	|	ВЫРАЗИТЬ(ВЫБОР 
	|		КОГДА ВЫРАЗИТЬ(ЛЕВ(Стат.Основание, 2) КАК СТРОКА(2)) = ""iq"" ТОГДА ПОДСТРОКА(Стат.Основание, 4, 36)
	|	    КОГДА ВЫРАЗИТЬ(ЛЕВ(Стат.Основание, 2) КАК СТРОКА(2))= ""mc"" ТОГДА ПОДСТРОКА(Стат.Основание, 4, ДЛИНАСТРОКИ(Стат.Основание)-3 ) 
	|	ИНАЧЕ ""не"" КОНЕЦ КАК СТРОКА(36))КАК ИдОснования
	|ПОМЕСТИТЬ ВТ051
	|ИЗ
	...
	|		ПО ВТ05.Сигнатура = Стат.Сигнатура 
	|; 
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ05.ДатаЗвонка КАК ДатаЗвонка,
	...
	|	ВТ05.Принят КАК Принят,
	|	ВТ05.Направление КАК Направление, 
	|	ВТ05.ИдОснования КАК Основание,
	|	ЕСТЬNULL(Интернет.Инициатор, ЕСТЬNULL(ЗвонокОснование.Инициатор.Код + "" "" + ЗвонокОснование.Инициатор.Назначение, КаналНомерНазначение)) КАК Канал
	|ПОМЕСТИТЬ ВТ052
	|ИЗ
	|	ВТ051  КАК ВТ05
	//Если звонок основан на интернет-заявке, в поле канал выведи сайт
	//----------------------------------------------------------------
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИнтернетЗаявки КАК Интернет
	|		ПО Интернет.Сигнатура = ВТ05.ИдОснования        
	//Если это исходящий звонок по пропущенному не по интернет-зявке, в поле канал выведи канал пропущенного
	//-------------------------------------------------------------------------------------------------------
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Звонки КАК ЗвонокОснование
	|		ПО ЗвонокОснование.Сигнатура  = ВТ05.ИдОснования
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТ05.Канал КАК Канал,  
	|	ВТ05.Основание КАК Основание,
