...
Процедура ПриЗагрузкеВариантаНаСервере(Настройки)
	...
	ПредПериод = Новый СтандартныйПериод();
	ПредПериод.ДатаНачала 	= НачалоМесяца(ДобавитьМесяц(ТекДата,-1));
	ПредПериод.ДатаОкончания = КонецМесяца(ДобавитьМесяц(ТекДата,-1));	
	ЭтаФорма.Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ПредПериод" ,ПредПериод);
КонецПроцедуры
