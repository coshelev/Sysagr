...
Процедура УстановиОтборы()
	...
	Если Не ПараметрыСеанса.РольДоступнаАдминистратор Тогда
		...
		|ИЗ
		|					Справочник.Предприятие КАК П ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТочкиПродажВподразделения КАК ТвП ПО П.Ссылка = ТвП.Подразделение
		|ГДЕ
		|					П.Ссылка В ИЕРАРХИИ (ВЫБРАТЬ РАЗЛИЧНЫЕ Польз.Подразделение ИЗ Справочник.Пользователи.Подразделения КАК Польз ГДЕ Польз.Ссылка = &Ссылка)";
