...
Функция СводныйГАЗ()
	ТекстЗапроса = Сводный();
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "2 = 2", "Продажи.Проект ПОДОБНО ""ГАЗ%"" ");
	Возврат ТекстЗапроса
КонецФункции

Функция СводныйУАЗ()
	ТекстЗапроса = Сводный();
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "2 = 2", "Продажи.Проект ПОДОБНО ""УАЗ%"" ");
	Возврат ТекстЗапроса
КонецФункции

Функция СводныйFord()
	ТекстЗапроса = Сводный();
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "2 = 2", "Продажи.Проект ПОДОБНО ""Ford%"" ");
	Возврат ТекстЗапроса
КонецФункции

Функция СводныйChery()
	ТекстЗапроса = Сводный();
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "2 = 2", "Продажи.Проект ПОДОБНО ""CHERY%"" ");
	Возврат ТекстЗапроса
КонецФункции 

Функция СводныйКАМАЗ()
	ТекстЗапроса = Сводный();
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "2 = 2", "Продажи.Проект ПОДОБНО ""КАМАЗ%"" ");
	Возврат ТекстЗапроса
КонецФункции
...
Функция Сводный()
	...
	|ГДЕ
	|	ЗадачаЛояльности.Тип = ЗНАЧЕНИЕ(Перечисление.ТипЗадачи.ЛояльностьПродажАвто)
	...
	|				И Вопрос В (&_1йБазовыйВопросЛояльности, &_2йБазовыйВопросЛояльности)) КАК ВТ01_АнкетыРасш
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СигнатураЗадачи,
	|	Вопрос,
	...
	|	ВЫБОР
	|		КОГДА ЗадачаЛояльности.ПричинаЗакрытия = """"
	|				И (_1йБазовыйВопросЛояльности.Ответ В (&_1, &_2, &_3, &Нет)
	...
	|	ВЫБОР
	|		КОГДА ЗадачаЛояльности.ПричинаЗакрытия = """"
	|				И НЕ _1йБазовыйВопросЛояльности.Ответ В (&_1, &_2, &_3, &Нет)
	|				И НЕ _2йБазовыйВопросЛояльности.Ответ В (&_1, &_2, &_3)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ01_АнкетыРасш КАК _1йБазовыйВопросЛояльности
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ01_АнкетыРасш КАК _2йБазовыйВопросЛояльности
	...
	|ИЗ
	|	ВТ03 КАК ВТ03
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ БизнесПроцесс.ОбратнаяСвязьЛояльностиПродаж КАК БП
	...
	|	ВТ05.ЗначениеПараметра КАК ЗначениеПараметра,
	|	ВТ05.ЗначениеДопПараметра КАК ЗначениеДопПараметра,
	|	Продажи.Проект КАК УчетнаяПрограмма
	|ИЗ
	|	ВТ05 КАК ВТ05
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Продажи КАК Продажи
...
Функция Детальный()
	...
	|			И (ДовольныЛиВыКачеством.Вопрос В (&_1йБазовыйВопросЛояльности))
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ01_АнкетыРасш КАК ДостоинЛиНашСервис
	|		ПО ВТ00.СигнатураПродажи = ДостоинЛиНашСервис.СигнатураЗадачи
	|			И (ДостоинЛиНашСервис.Вопрос В (&_2йБазовыйВопросЛояльности))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
