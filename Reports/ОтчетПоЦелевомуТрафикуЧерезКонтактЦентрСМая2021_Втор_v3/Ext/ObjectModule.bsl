...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Праздники.Колонки.Добавить("Дата", Новый ОписаниеТипов("Дата",,,,, Новый КвалификаторыДаты(ЧастиДаты.Дата)));
	Запрос = Новый Запрос("ВЫБРАТЬ Дата ИЗ РегистрСведений.Праздники ГДЕ Дата МЕЖДУ &Начало и &Окончание"); 
	Запрос.УстановитьПараметр("Начало", 	ДатаНачала); 
	Запрос.УстановитьПараметр("Окончание", 	ДатаОкончания);
	РезЗапроса 	= Запрос.Выполнить();     	
	Праздники 	= ?(РезЗапроса.Пустой(), Праздники, РезЗапроса.Выгрузить()); 

	//Определи количество дней
	//------------------------
	Начало 				= НачалоДня(ДатаНачала);
	...
	Пока Начало <= Окончание Цикл	
		...
		//Праздничный день признаем выходным (седьмым днем недели)
		ДеньНедели = ?(Праздники.Найти(Начало)<>Неопределено, 7, ДеньНедели); 
	...
	ВыходныеСтрокой				= "";	//Для выделения даты красным в отчете
	Пока Начало <= КонецМесяца(Окончание) Цикл		
		...
		ВыходныеСтрокой				= ?(ДеньНедели > 5, ВыходныеСтрокой+";"+Формат(Начало,"ДФ=dd.MM.yyyy"), ВыходныеСтрокой);	
		Начало						= Начало + 1*24*60*60;
	КонецЦикла;   
	
	//Сообщить(СтрШаблон("КоличествоБудней = %1", КоличествоБуднейВМесяце));
	//Сообщить(СтрШаблон("КоличествоВыходныхВМесяце = %1",КоличествоВыходныхВМесяце)); 
	...
	Даты.Колонки.Добавить("ВсеВыходные",				Новый ОписаниеТипов("Строка")); 	//Для выделения даты красным в отчете 
	...
	Зап.ВсеВыходные					= ВыходныеСтрокой; 
	
	ТЗ = РезультатЗапроса.Выгрузить();
	ТЗ.Колонки.Добавить("ЭтоВыходной", Новый ОписаниеТипов("Булево")); 
	Для Каждого i Из ТЗ Цикл
		Доступно = 	ДеньНедели(i.ДатаЗвонка)>5 
						Или Праздники.Найти(i.ДатаЗвонка) <> Неопределено;  
						
		i.ЭтоВыходной =	Доступно;
		//Сообщить(СтрШаблон ("%1 %2", i.ЭтоВыходной, ТипЗнч(i.ЭтоВыходной)));
	КонецЦикла;
	...
	//-------------------------------
	Настройки.ПараметрыДанных.Элементы.Найти("ДатаНачала2").Значение 				= ДатаНачала; 					Настройки.ПараметрыДанных.Элементы.Найти("ДатаНачала2").Использование				= Истина;
	Настройки.ПараметрыДанных.Элементы.Найти("ДатаОкончания2").Значение 			= ДатаОкончания;                Настройки.ПараметрыДанных.Элементы.Найти("ДатаОкончания2").Использование 			= Истина;     
		
	Настройки.ПараметрыДанных.Элементы.Найти("КоличествоБудней").Значение 			= КоличествоБудней;				Настройки.ПараметрыДанных.Элементы.Найти("КоличествоБудней").Использование			= Истина;
	Настройки.ПараметрыДанных.Элементы.Найти("КоличествоВыходных").Значение 		= КоличествоВыходных;			Настройки.ПараметрыДанных.Элементы.Найти("КоличествоВыходных").Использование 		= Истина;  

	Настройки.ПараметрыДанных.Элементы.Найти("КоличествоДней").Значение 			= КоличествоДней;				Настройки.ПараметрыДанных.Элементы.Найти("КоличествоДней").Использование			= Истина;

	
	Настройки.ПараметрыДанных.Элементы.Найти("КоличествоБуднейВМесяце").Значение 	= КоличествоБуднейВМесяце;		Настройки.ПараметрыДанных.Элементы.Найти("КоличествоБуднейВМесяце").Использование	= Истина;
	Настройки.ПараметрыДанных.Элементы.Найти("КоличествоВыходныхВМесяце").Значение 	= КоличествоВыходныхВМесяце;	Настройки.ПараметрыДанных.Элементы.Найти("КоличествоВыходныхВМесяце").Использование = Истина;  
