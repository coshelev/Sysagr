...
Процедура УстановиОтборы()
	...
	Запрос.УстановитьПараметр("атаДок", ТекущаяДата());	
	...
	Если Не ПараметрыСеанса.РольДоступнаАдминистратор Тогда
		...
		Запрос = Новый Запрос();
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ	ВЫРАЗИТЬ(ТвП.Точка КАК СТРОКА(32)) КАК Точка
		|ИЗ 				Справочник.Предприятие КАК П ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТочкиПродажВподразделения КАК ТвП ПО П.Ссылка = ТвП.Подразделение
		|ГДЕ				П.Ссылка В ИЕРАРХИИ (ВЫБРАТЬ РАЗЛИЧНЫЕ Польз.Подразделение ИЗ Справочник.Пользователи.Подразделения КАК Польз ГДЕ Польз.Ссылка = &Ссылка)";
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Т.ХарактеристикаОбъекта КАК ВидРемонта
	|ИЗ
	|	РегистрСведений.Продажи КАК Т
	|ГДЕ
	|	Т.Вид = &Вид
	|	И Т.Дата МЕЖДУ ДОБАВИТЬКДАТЕ(&ДатаДок, МЕСЯЦ, -3) И &ДатаДок
	|
	|УПОРЯДОЧИТЬ ПО
	|	Т.ХарактеристикаОбъекта";
	Запрос.УстановитьПараметр("Вид", "Ремонт авто") ;
