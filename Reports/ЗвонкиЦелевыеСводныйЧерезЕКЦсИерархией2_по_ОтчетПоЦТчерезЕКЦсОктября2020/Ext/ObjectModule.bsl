...
//Выполни запрос в базе ОУ по выставкам и встречам
	...
	|			ИЛИ ВТ05.ТочкаЦелевая.Родитель В ИЕРАРХИИ (&ГруппаЦелевыхТочек1))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТЗ.ДатаЗвонка КАК ДатаЗвонка,
	|	ТЗ.Сигнатура КАК Сигнатура,
	|	ТЗ.ТочкаЦелевая КАК ТочкаЦелевая,
	|	ТЗ.КоличествоЦелевых КАК КоличествоЦелевых,
	|	ТЗ.Канал КАК Канал,
	|	ТЗ.ИнтернетЗаявка КАК ИнтернетЗаявка,
	|	ТЗ.ИнициаторИнтернетЗаявки КАК ИнициаторИнтернетЗаявки,
	|	ТЗ.КаналИлиСайт КАК КаналИлиСайт,
	|	ТЗ.КаналИлиСайтПредставление КАК КаналИлиСайтПредставление,
	|	ТЗ.ИерархияГруппаНижняя КАК ИерархияГруппаНижняя,
	|	ТЗ.ИерархияГруппаВерхняя КАК ИерархияГруппаВерхняя,
	|	ТЗ.ИерархияГруппаВерхняяСортировка КАК ИерархияГруппаВерхняяСортировка
	|ПОМЕСТИТЬ ВТ08
	|ИЗ
	|	&ТЗ КАК ТЗ
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ07.ДатаЗвонка КАК ДатаЗвонка,
	|	ВТ07.Сигнатура КАК Сигнатура,
	|	ВТ07.ТочкаЦелевая КАК ТочкаЦелевая,
	|	ВТ07.КоличествоЦелевых КАК КоличествоЦелевых,
	|	ВТ07.Канал КАК Канал,
	|	ВТ07.ИнтернетЗаявка КАК ИнтернетЗаявка,
	|	ВТ07.ИнициаторИнтернетЗаявки КАК ИнициаторИнтернетЗаявки,
	|	ВТ07.КаналИлиСайт КАК КаналИлиСайт,
	|	ВТ07.КаналИлиСайтПредставление КАК КаналИлиСайтПредставление,
	|	ВТ07.ИерархияГруппаНижняя КАК ИерархияГруппаНижняя,
	|	ВТ07.ИерархияГруппаВерхняя КАК ИерархияГруппаВерхняя,
	|	ВТ07.ИерархияГруппаВерхняяСортировка КАК ИерархияГруппаВерхняяСортировка
	|ИЗ
	|	ВТ07 КАК ВТ07
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ВТ08.Сигнатура,
	|	ВТ08.ТочкаЦелевая,
	|	ВТ08.КоличествоЦелевых,
	|	ВТ08.Канал,
	|	ВТ08.ИнтернетЗаявка,
	|	ВТ08.ИнициаторИнтернетЗаявки,
	|	ВТ08.КаналИлиСайт,
	|	ВТ08.КаналИлиСайтПредставление,
	|	ВТ08.ИерархияГруппаНижняя,
	|	ВТ08.ИерархияГруппаВерхняя,
	|	ВТ08.ИерархияГруппаВерхняяСортировка
	|ИЗ
	|	ВТ08 КАК ВТ08";
	...
	Запрос.УстановитьПараметр("ГруппаЦелевыхТочек",			Справочники.ТочкиЦелевые.НайтиПоНаименованию("Автомобили с пробегом"));
	Запрос.УстановитьПараметр("ГруппаЦелевыхТочек1",		Справочники.ТочкиЦелевые.НайтиПоНаименованию("Луидор-автосалоны"));
	Запрос.УстановитьПараметр("ИерархияМаркетинга",			ИерархияМаркетинга);

	ОтчСервис = Отчеты.ЗвонкиЦелевыеСервисЧерезЕКЦсИерархией2.Создать();
	ЗаполнитьЗначенияСвойств(ОтчСервис, ЭтотОбъект);
	тзСервис = ОтчСервис.РезультатЗапроса().Выгрузить();
	
	//тзСервис.Очистить(); //для отладки
	
	Запрос.УстановитьПараметр("ТЗ",тзСервис);
...
Функция РеестрЦелевых_Загрузить(БИ, ИмяТаблЧасти, ДатаНачала, ДатаОкончания, КодТочкиОформленияКорректировки, ПустаяСтрока) Экспорт
	...
	БИ_Запрос.Текст = СтрЗаменить(БИ_Запрос.Текст, "И Звонки.ТочкаОформления.ВидДеятельности = ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.ПродажаНовые)", "И Звонки.ТочкаОформления.ВидДеятельности В (ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.ПродажаНовые), ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.Вторичные))");
	БИ_Запрос.Текст = СтрЗаменить(БИ_Запрос.Текст, "И ЗвонокОснование.ТочкаОформления.ВидДеятельности = ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.ПродажаНовые)", "И ЗвонокОснование.ТочкаОформления.ВидДеятельности В (ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.ПродажаНовые), ЗНАЧЕНИЕ(Справочник.ВидыДеятельности.Вторичные))");
