...
Функция СформироватьОтчет(ТаблДок, СигнатураПродажи = "") Экспорт
	...
	|				И ВТ02_АнкетыЗадачБезФильтраПоВопросам.Ответ В (&Ответ1, &Ответ2, &Ответ3, &ОтветНет)
	...
	Если  ПакетРезультатов[4].Пустой() Тогда
		Возврат ""; 
	КонецЕсли;
	...
	ВыбТочка = ПакетРезультатов[4].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
