...
Процедура Сформировать(Команда)
	...
	Для Каждого Стр Из ЭтаФорма.Отчет.ИсходныеДанныеИзФайла Цикл
		...
		//Удали строки со статусом "Ошибка" и "Не доставлено"
		//---------------------------------------------------
		Стат = Стр.Статус;
		Если СтрНайти(Стат, "Ошибка") Тогда
			Сооб = Новый СообщениеПользователю();
			Сооб.Текст = "елефон: "+Тел+ ", удаляем, статус Ошибка";  
			Сооб.Сообщить();
			Продолжить
		КонецЕсли;
		Если СтрНайти(Стат, "Не доставлено") Тогда
			Сооб = Новый СообщениеПользователю();
			Сооб.Текст = "елефон: "+Тел+ ", удаляем, статус Не доставлено";  
			Сооб.Сообщить();
			Продолжить
		КонецЕсли;
		...
		//Удали строку с неправильной датой
		//----------------------------------
		Попытка
