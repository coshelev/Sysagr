...
Функция CURL_HttpResponse(Стр, ИмяФайлаДвДанных="") Экспорт 
	...
	//Пример: curl --request POST 'https://luidor-expert.ru/api/external/1c/chat/116316fe-2185-4b3f-a6a6-8d79dff7fd09/' -H 'Content-Type:application/json' -d '{"type":"text", "message":{"text":"Спасибо за обращение, принято в работу"}}'
	...
	//Выдели порт сервера, например из строки https://api.direct.yandex.com/json/v5/report:443
	port		 			= "";
	ПозицияДвоеточияПорта 	= СтрНайти(url, ":"); 
	Если ПозицияДвоеточияПорта > 0 Тогда
	...
	//Обработай опцию -H или --header (заголовки). Их может быть несколько в одной команде, 
	//например  curl -X POST 'https://apiauto.ru/1.0/auth/login' -H 'x-authorization:cheb003...' -H 'Content-Type:application/json' -d '{"login":"Lui...","password":"lui..."}'   
	...
	Для i = 1 По СтрДлина(_Стр) Цикл
		...
		Если lastApostrophe_pos = 0 Тогда
			Прервать;
		КонецЕсли;
		КлючЗначениеСтрокой = Сред(_Стр, Н_pos+4, lastApostrophe_pos -  (Н_pos+3));
	...
	_Стр = СтрЗаменить(_Стр, "--data '{", "-d {");  
	_Стр = СтрЗаменить(_Стр, "-d '{", "-d {");  
	_Стр = ?(Прав(_Стр, 2) = "}'", СтрЗаменить(_Стр, "}'", "}"), _Стр);				
		
	d_pos 				= СтрНайти(_Стр, "-d {");  
	lastCurlyBrace_pos 	= ?(d_pos > 0, СтрНайти(_Стр, "}", НаправлениеПоиска.СКонца,СтрДлина(_Стр),1),0); 
	ТелоЗапроса 		= ?(lastCurlyBrace_pos > 0, Сред(_Стр, d_pos+3, lastCurlyBrace_pos - (d_pos+2) ), "");
