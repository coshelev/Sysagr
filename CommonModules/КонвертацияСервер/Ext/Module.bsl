...
Функция РегионПолучитьПоНомеруТелефона(НомерТелефона) Экспорт

// Функция возвращает ссылку на элемент справочника "Страны" по переданному номеру телефона
// Если запись в справочнике не найдена, то функция возвращает пустую ссылку
//-------------------------------------------------------------------------------------------------
	Доступно = ЗначениеЗаполнено(НомерТелефона);
	Доступно = Доступно И (ТипЗнч(НомерТелефона) = Тип("Строка"));
...
// Получим нормализованный формат переданного номера телефона
//-------------------------------------------------------------------------------------------------
	Если (Доступно = Истина) Тогда
		ИскомыйНомер = ТелефонВнешнийНормализовать(НомерТелефона);

		Если (СтрДлина(ИскомыйНомер) = 10) Тогда
			ИскомыйНомер = "7" + СокрЛП(ИскомыйНомер);
		ИначеЕсли (СтрДлина(ИскомыйНомер) = 11) Тогда
			Доступно = (Лев(ИскомыйНомер,4) = "8800");
			Доступно = Доступно ИЛИ (Лев(ИскомыйНомер,4) = "8804");
			Доступно = Доступно ИЛИ (Лев(ИскомыйНомер,4) = "8808");

			Если (Доступно = Истина) Тогда
				ИскомыйНомер = "7" + Прав(СокрЛП(ИскомыйНомер),10);
			КонецЕсли;
		КонецЕсли;

// Выполним поиск в справочнике "Страны"
//-------------------------------------------------------------------------------------------------
		ТекстЗапроса = "
		|SELECT	TOP 1 Страна
		|FROM	РегистрСведений.ТелНомерныеПланы
		|WHERE	(НачДиапазона <= &ИскомыйНомер) И (КонДиапазона >= &ИскомыйНомер)
		|ORDER	BY Страна.ЭтоГруппа ASC";

		Запрос = Новый Запрос;
		Запрос.Текст = ТекстЗапроса;
		Запрос.УстановитьПараметр("ИскомыйНомер",Число(ИскомыйНомер));
		Результат = Запрос.Выполнить().Выбрать();

		Если (Результат.Следующий()) Тогда
			Ответ = Результат.Страна;
		КонецЕсли;
	КонецЕсли;
