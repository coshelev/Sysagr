...
Функция ИнтернетЗаявка(PhoneNum, Site, Extra = "", СигнатураПочтовогоСообщения = "") Экспорт
	...
	//--------------------------------------------------------------------------------------------
	Выполнено = РегистрыСведений.ИнтернетЗаявки.Добавить(Сигнатура, ТекДата, Site, PhoneNum, Extra, СигнатураПочтовогоСообщения);
	...
	Если СтрДлина(Телефон)>=10 Тогда
		...
		|	Т.Страна КАК Страна
		|ИЗ
		|	РегистрСведений.ТелНомерныеПланы КАК Т
		|ГДЕ
		|	Т.НачДиапазона >= &Телефон
		|	И Т.КонДиапазона <= &Телефон
		|	И Т.Страна.Наименование <> &Наименование";
