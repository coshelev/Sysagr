...
Функция ШаблонURL1Метод1(Запрос)
	...
	Доступно = (СтрНайти(ТелоЗапроса, "5f5b56ffbc46f80001d5a8d4") Или СтрНайти(ТелоЗапроса, "5f5b574519635b0001b5df97"));
	Если Доступно Тогда
	...
	Рез = "empty";
	Доступно = ТипЗнч(Телефон) = Тип("Строка") И ЗначениеЗаполнено(Телефон);
	Если Доступно Тогда			
		//Создать интернет-заявку
		ТелоОтвета = ВэбСервисы.ИнтернетЗаявка(Телефон, Сайт, ДопИнфо);
		Ответ.УстановитьТелоИзСтроки(ТелоОтвета);
		Возврат Ответ;
	КонецЕсли;
	...
	Доступно = 	Сайт = "chery-luidor.ru" И СтрНайти(_ТелоЗапроса, "Array")>0 И	НачалоТел > 0;
	Если Доступно Тогда
